#!/usr/bin/env python3
"""Read-only review-loop closure check for the stock system.

This gate narrows the consolidation index to the `review_loop` lane. It keeps
review tasks, result records, reminders, reports, and learning panels visible
as a closed feedback loop before any construction work consolidates assets.
"""
...
except ModuleNotFoundError:  # Running as `python ci/stock_review_loop_ci_check.py`.
...
REQUIRED_SAMPLE_TERMS = ["任务", "结果", "提醒", "报告", "闭环"]
GUARDRAILS = [
    "read_only_review_loop_check",
...
def review_lane(panel: dict[str, Any]) -> dict[str, Any] | None:
...
def review_topic_workstreams(index: dict[str, Any]) -> list[dict[str, Any]]:
    return [
        item
...
def review_next_queue(index: dict[str, Any]) -> list[dict[str, Any]]:
    return [
        item
...
def validate_report(report: dict[str, Any]) -> list[str]:
    ...
    if not lane:
        problems.append("missing_review_loop_lane")
    elif lane.get("queue_count", 0) < 1:
        problems.append(f"review_loop_queue_too_small:{lane.get('queue_count', 0)}")
...
def build_review_loop_report() -> dict[str, Any]:
    index = consolidation.build_consolidation_index()
    panel = closure_panel.build_closure_panel()
    report = {
        "name": "stock_review_loop_closure",
        "owner_lane": OWNER_LANE,
        "lane": review_lane(panel),
        "next_queue": review_next_queue(index),
        "topic_workstreams": review_topic_workstreams(index),
...
def main() -> int:
    ...
    report = build_review_loop_report()
    if args.json:
    ...
    if report["ci_gate_status"] != "pass":
        print("FAIL: stock review loop closure found blocking problems", file=sys.stderr)
        return 1
    return 0
