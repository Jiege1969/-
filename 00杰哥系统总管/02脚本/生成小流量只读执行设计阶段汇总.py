"""
名称：生成小流量只读执行设计阶段汇总.py
作用：汇总小流量只读执行方案、禁用态、观测闭环和批次放行单，形成真实小流量前的阶段门。
触发方式：python 生成小流量只读执行设计阶段汇总.py
依赖：Python 标准库；小流量只读执行相关验收日志；最新v3总体验收日志。
所属系统：00杰哥系统总管
安全边界：只读取验收日志并生成阶段汇总；不联网；不写库；不生成正式文档；不真实渲染；不真实转换；不真实发送企业微信；不触发n8n；不接入税收；不写入旧系统。
创建/修改记录：2026-04-27 创建小流量只读执行设计阶段汇总脚本。
"""
...
def main() -> int:
    ...
    checks = [
        latest_check(log_root, "小流量禁用态完成度", "readonly-disabled-completion-verify-*.json"),
        latest_check(log_root, "小流量执行方案", "readonly-execution-plan-verify-*.json"),
        ...
        latest_check(log_root, "观测闭环", "readonly-observation-loop-verify-*.json"),
        latest_check(log_root, "批次放行单", "readonly-batch-release-form-verify-*.json"),
    ]
    ...
    report = {
        ...
        "阶段": "小流量只读执行设计阶段汇总",
        "总体验收": {"路径": str(acceptance_path), "汇总": acceptance.get("summary", {})},
        "阶段链路": checks,
        "汇总": {
            "链路总数": len(checks),
        ...
        "阶段结论": "小流量只读执行设计阶段完成；仍需具体批次人工确认后才可执行真实动作。" if not failed_items else "小流量只读执行设计阶段未完成",
        "下一阶段": "具体批次人工确认与真实小流量动作前最终放行",
        "仍然关闭": [
            "真实联网",
            "正式库写入",
            "正式文档输出",
    ...
    output = output_dir / "小流量只读执行设计阶段汇总_最新.json"
    latest = output_dir / "小流量只读执行设计阶段汇总_最新.json"
