"""
名称：验证小流量只读执行观测闭环.py
作用：生成并验证小流量只读执行观测闭环汇总，确认执行方案、前快照、后观测和回滚确认单均通过。
触发方式：python 验证小流量只读执行观测闭环.py
依赖：Python 标准库；生成小流量只读执行观测闭环汇总.py。
所属系统：00杰哥系统总管
安全边界：只生成和验证汇总；不联网；不写库；不生成正式文档；不真实渲染；不真实转换；不真实发送企业微信；不触发n8n；不接入税收；不写入旧系统。
创建/修改记录：2026-04-27 创建小流量只读执行观测闭环验收脚本。
"""
...
def main() -> int:
    ...
    script = manager / "02脚本" / "生成小流量只读执行观测闭环汇总.py"
    result = subprocess.run([sys.executable, str(script)], capture_output=True, text=True, encoding="utf-8", errors="replace", timeout=120)
    summary_path = manager / "03数据" / "小流量只读执行" / "小流量只读执行观测闭环汇总_最新.json"
    summary = load_json(summary_path) if summary_path.exists() else {}
    closed = summary.get("仍然关闭", [])
    checks = [
        check("闭环汇总生成成功", result.returncode == 0, result.stdout.strip() or result.stderr.strip()),
        check("闭环汇总文件存在", summary_path.exists(), str(summary_path)),
        check("闭环链路数量完整", summary.get("汇总", {}).get("链路总数") == 4, summary.get("汇总", {})),
        check("闭环全部通过", summary.get("汇总", {}).get("失败") == 0, summary.get("汇总", {})),
    ...
    verify = {
        ...
        "检查类型": "readonly-observation-loop-verify",
        "汇总": {
    ...
    output = output_dir / f"readonly-observation-loop-verify-{datetime.now().strftime('%Y%m%d-%H%M%S')}.json"
    latest = output_dir / "readonly-observation-loop-verify-最新.json"
