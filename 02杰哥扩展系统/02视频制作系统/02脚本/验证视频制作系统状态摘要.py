...
"""
名称：验证视频制作系统状态摘要.py
作用：验证视频制作系统状态摘要生成链路可运行，并确认剪辑软件调用、真实渲染、n8n触发和企业微信发送仍关闭。
触发方式：python 验证视频制作系统状态摘要.py
依赖：Python标准库；生成视频制作系统状态摘要.py。
所属系统：02杰哥扩展系统/02视频制作系统
安全边界：只运行本地状态摘要生成和验收；只写入04日志；不调用剪辑软件；不执行真实渲染；不触发n8n；不发送企业微信；不写旧系统。
创建/修改记录：2026-04-29 创建视频制作系统状态摘要验收脚本。
标识：video-production-system-status-summary-verify
"""
...
def main() -> int:
    root = module_root()
    generator = root / "02脚本" / "生成视频制作系统状态摘要.py"
    latest_json = root / "03数据" / "09状态摘要" / "video-production-system-status-summary-最新.json"
    latest_md = root / "03数据" / "09状态摘要" / "视频制作系统状态摘要_最新.md"
    log_dir = root / "04日志"
    ...
    add_check(checks, "未调用剪辑软件", safety.get("调用剪辑软件") is False, safety)
    add_check(checks, "未执行真实渲染", safety.get("执行真实渲染") is False, safety)
    ...
    add_check(checks, "未接入交易", safety.get("接入交易") is False, safety)

    passed = sum(1 for item in checks if item["通过"])
    failed = len(checks) - passed
    verify_report = {
        "生成时间": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        "类型": "video-production-system-status-summary-verify",
        "汇总": {"通过": passed, "失败": failed},
        "检查项": checks,
    }
    output = log_dir / "video-production-system-status-summary-verify-最新.json"
