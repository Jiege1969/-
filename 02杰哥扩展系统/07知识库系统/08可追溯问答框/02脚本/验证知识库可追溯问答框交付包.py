...
def main() -> int:
    root = module_root()
    public_source_rule = Path("D:/杰哥智能化系统/02杰哥扩展系统/00公共组件/01配置/信息来源可信度分层规则.json")
    ...
    checks: list[dict[str, Any]] = [
        ...
        check("公共来源可信度规则存在", public_source_rule.exists(), str(public_source_rule)),
    ]
    ...
    answered_items = [item for item in qa_items if item.get("状态") == "answered_with_sources"]
    downgraded_items = [item for item in qa_items if item.get("状态") == "no_evidence_downgraded"]
    ...
    checks.extend([
        ...
        check("有来源回答必须包含证据卡", all(len(item.get("证据卡", [])) >= 1 for item in answered_items), len(answered_items)),
        check("无证据问题必须明确降级", bool(answered_items) or bool(downgraded_items), {"有来源回答": len(answered_items), "无证据降级": len(downgraded_items)}),
    ...
    for item in qa_items:
        ...
        for evidence in evidences:
            ...
            for field in ["可信度等级", "来源类型", "来源用途边界", "是否需要反向追溯", "是否可作正式依据候选"]:
                checks.append(check(f"证据包含公共来源判断字段：{evidence_name}/{field}", field in evidence and evidence.get(field) not in ("", None), evidence.get(field)))
