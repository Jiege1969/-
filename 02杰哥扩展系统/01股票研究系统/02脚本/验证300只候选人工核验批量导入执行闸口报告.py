# -*- coding: utf-8 -*-
"""
名称：验证300只候选人工核验批量导入执行闸口报告.py
作用：验证121执行闸口当前会阻断空表导入，且不修改103。
触发方式：python 验证300只候选人工核验批量导入执行闸口报告.py
依赖：Python标准库；生成300只候选人工核验批量导入执行闸口报告.py；300只候选人工核验批量导入执行闸口规则.json。
所属系统：02杰哥扩展系统/01股票研究系统
安全边界：只读本地产物并写验收日志；不写入核验结果；不修改103；不导入填报表；不应用103派生任务；不刷新101/102；不触发发送链路；不触发n8n；不发送企业微信；不写正式库；不调用券商接口；不自动交易；不重启服务。
创建/修改记录：2026-05-01 创建人工核验批量导入执行闸口验收脚本。
标识：stock-trial-pool-300-manual-verification-batch-import-execution-gate-verify
"""
...
def main() -> int:
    ...
    script = root / "02脚本" / "生成300只候选人工核验批量导入执行闸口报告.py"
    ...
    checks.append(check("当前阻断导入103", gate.get("是否允许进入人工确认导入103") is False, gate))
    checks.append(check("阻断原因包含无已填写行", any("无已填写行" in item for item in gate.get("阻断原因", [])), gate.get("阻断原因", [])))
    checks.append(check("119摘要存在", bool(report.get("119校验摘要")), report.get("119校验摘要", {})))
    checks.append(check("120摘要存在", bool(report.get("120预演摘要")), report.get("120预演摘要", {})))
    ...
    checks.append(check("Markdown包含闸口结论", "是否允许进入人工确认导入103" in text and "阻断原因" in text, str(latest_md)))
    passed = sum(1 for item in checks if item["通过"])
    failed = len(checks) - passed
    result = {"生成时间": datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "通过": passed, "失败": failed, "检查项": checks, "输出文件": str(latest_json)}
    log_dir = root / "04日志" / "人工核验批量导入执行闸口"
    stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output = log_dir / f"trial-pool-300-manual-verification-batch-import-execution-gate-verify-{stamp}.json"
    latest_log = log_dir / "trial-pool-300-manual-verification-batch-import-execution-gate-verify-最新.json"
