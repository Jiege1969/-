# -*- coding: utf-8 -*-
"""
名称：验证300只候选真实发送前检查包.py
作用：验证真实发送前检查包是否可生成、当前是否阻断真实发送讨论资格、是否不执行真实发送。
触发方式：python 验证300只候选真实发送前检查包.py
依赖：Python标准库；生成300只候选真实发送前检查包.py；300只候选真实发送前检查规则.json。
所属系统：02杰哥扩展系统/01股票研究系统
安全边界：只读本地产物并写入验收日志；不触发发送链路；不触发n8n；不调用OpenClaw；不发送企业微信；不写正式库；不写旧系统；不调用券商接口；不自动交易；不重启服务。
创建/修改记录：2026-04-30 创建300只候选真实发送前检查包验收脚本。
标识：stock-trial-pool-300-pre-real-send-check-verify
"""
...
def main() -> int:
    root = module_root()
    script = root / "02脚本" / "生成300只候选真实发送前检查包.py"
    ...
    checks.append(check("当前阻断真实发送讨论资格", report.get("是否具备提交真实发送讨论资格") is False, report.get("阻断原因", [])))
    ...
    checks.append(check("Markdown包含阻断原因和安全边界", "阻断原因" in text and "安全边界" in text, "真实发送前检查报告"))
    passed = sum(1 for item in checks if item["通过"])
    failed = len(checks) - passed
    result = {"生成时间": datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "通过": passed, "失败": failed, "检查项": checks, "输出文件": str(latest)}
    output_dir = root / "04日志" / "真实发送前检查"
    stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output = output_dir / f"trial-pool-300-pre-real-send-check-verify-{stamp}.json"
    latest_log = output_dir / "trial-pool-300-pre-real-send-check-verify-最新.json"
