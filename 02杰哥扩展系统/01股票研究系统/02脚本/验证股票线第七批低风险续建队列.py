...
RESULT_JSON = DATA_DIR / "股票线第七批低风险续建队列验收_最新.json"
RESULT_MD = DATA_DIR / "股票线第七批低风险续建队列验收_最新.md"
...
def main() -> None:
    ...
    if len(blocked) < 3:
        errors.append("必须登记企业微信、正式配置和交易红线阻断项")
    ...
    if summary.get("queue_count") != len(construction):
        errors.append("queue_count与施工队列数量不一致")
    if summary.get("blocked_count") != len(blocked):
        errors.append("blocked_count与阻断登记数量不一致")
