...
def main() -> None:
    ...
    for item in queue.get("construction_queue", []):
        if item.get("id") == "STOCK-AUTO5-001":
            item["status"] = "completed"
            item["validation_asset"] = "前台短答综合样例二次压缩验收结果_最新.json"
    ...
    lines = ["# 股票线第五批低风险续建队列", "", f"- 更新时间：{now}", f"- 已完成：{queue['summary']['completed_count']}", f"- 待推进：{queue['summary']['queued_count']}", "", "## 施工队列", ""]
