...
ASSET_PATH = DATA_DIR / "财报资金证据到前台短答压缩映射验收_最新.json"
RESULT_JSON = DATA_DIR / "财报资金证据到前台短答压缩映射验收结果_最新.json"
RESULT_MD = DATA_DIR / "财报资金证据到前台短答压缩映射验收结果_最新.md"
REQUIRED_CODES = {"002428", "002466", "688347", "000906", "300641"}
FORBIDDEN_PHRASES = ["必须操作", "确定获利", "满分", "无风险"]


def main() -> None:
    errors: list[str] = []
    asset = json.loads(ASSET_PATH.read_text(encoding="utf-8-sig")) if ASSET_PATH.exists() else {}
    if asset.get("asset_identity") != "W1财报资金证据到前台短答压缩映射验收":
        errors.append("资产身份不正确")
    if asset.get("status") != "shadow_mapping_acceptance":
        errors.append("状态不是shadow_mapping_acceptance")
    ...
    for item in records:
        ...
        if len(phrase) > 90:
            errors.append(f"{stock.get('code')}前台压缩话术过长")
        if not item.get("missing"):
            errors.append(f"{stock.get('code')}缺少missing")
        evidence = item.get("backend_evidence", {})
        if "financial" not in evidence or "capital" not in evidence:
            errors.append(f"{stock.get('code')}后台证据缺少financial/capital")
