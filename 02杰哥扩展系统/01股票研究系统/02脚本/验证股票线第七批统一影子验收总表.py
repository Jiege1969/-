...
ASSET_PATH = DATA_DIR / "股票线第七批统一影子验收总表_最新.json"
RESULT_JSON = DATA_DIR / "股票线第七批统一影子验收总表验收_最新.json"
RESULT_MD = DATA_DIR / "股票线第七批统一影子验收总表验收_最新.md"


def main() -> None:
    errors: list[str] = []
    asset = json.loads(ASSET_PATH.read_text(encoding="utf-8-sig")) if ASSET_PATH.exists() else {}
    if asset.get("asset_identity") != "W2第七批统一影子验收总表":
