...
"""
名称：生成行业景气核验预览.py
作用：校验行业景气人工核验模板，生成可进入前台行业表达的差异预览。
安全边界：只读核验模板；只写 03数据/179行业景气核验预览；不覆盖正式档案、不改推荐、不触发 n8n、不发送企业微信、不调用券商接口、不自动交易。
"""
...
def build_preview_item(item: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "名称": item.get("名称"),
        "行业": item.get("行业"),
        "当前景气估算": estimate.get("景气状态"),
        "正式行业景气判断": filled.get("正式行业景气判断"),
        "是否支持现有景气估算": filled.get("是否支持现有景气估算"),
        "样本估算偏差判断": filled.get("样本估算偏差判断"),
        "建议前台处理": filled.get("建议前台处理"),
        "核验摘要": filled.get("核验摘要"),
        "证据来源": {
            "来源名称": filled.get("行业指数或价格来源名称"),
            "来源URL": filled.get("行业指数或价格来源URL"),
            "数据日期": filled.get("数据日期"),
        },
        "核验人": item.get("核验人"),
        "核验日期": item.get("核验日期"),
        "备注": item.get("备注", ""),
...
def build_markdown(report: dict[str, Any]) -> str:
    ...
    for item in report["可预览记录"]:
        lines.extend([
            ...
            "",
            f"- 行业：{item['行业']}",
            f"- 当前景气估算：{item['当前景气估算']}",
            f"- 正式行业景气判断：{item['正式行业景气判断']}",
            f"- 是否支持现有景气估算：{item['是否支持现有景气估算']}",
            f"- 建议前台处理：{item['建议前台处理']}",
            f"- 核验摘要：{item['核验摘要']}",
            f"- 证据来源：{item['证据来源']['来源名称']} / {item['证据来源']['数据日期']}",
            "",
        ])
...
def main() -> int:
    ...
    template_path = root / "03数据" / "178行业景气人工核验模板" / "行业景气人工核验模板_最新.json"
    ...
    output_dir = root / "03数据" / "179行业景气核验预览"
    latest_json = output_dir / "行业景气核验预览_最新.json"
    latest_md = output_dir / "行业景气核验预览_最新.md"
    markdown = build_markdown(report)
    write_json(output_dir / f"行业景气核验预览_{stamp}.json", report)
    write_json(latest_json, report)
    write_text(output_dir / f"行业景气核验预览_{stamp}.md", markdown)
