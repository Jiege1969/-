# -*- coding: utf-8 -*-
"""
名称：验证单股证据核验191候选写入差异预览.py
作用：验证203候选写入差异预览可生成，且不覆盖原191 CSV、不写191台账。
触发方式：手动验收、股票系统日常一键运行或C+++总验收调用。
依赖：生成单股证据核验191候选写入差异预览.py、191人工填写CSV表单、202候选填写CSV副本。
所属系统：02杰哥扩展系统/01股票研究系统。
输出：04日志/单股证据核验191候选写入差异预览/single-stock-evidence-191-candidate-write-diff-preview-verify-最新.json。
安全边界：只运行203差异预览生成脚本并读取本地结果；不覆盖191 CSV，不写191台账，不写172/175/178，不写正式档案，不触发n8n，不发送企业微信，不调用券商接口，不自动交易，不更新施工接续包。
创建/修改记录：2026-05-03 创建203候选写入差异预览验证脚本。
标识：single-stock-evidence-191-candidate-write-diff-preview-verify
"""
...
def main() -> int:
    root = module_root()
    script = root / "02脚本" / "生成单股证据核验191候选写入差异预览.py"
    ...
    checks = [
        ...
        check(
            "候选可填入字段达到最小规模或已全部填入",
            int(summary.get("候选可填入字段数") or 0) >= 20
            or (int(summary.get("仍缺必填字段数") or 0) == 0 and int(summary.get("原已填写且与候选一致数") or 0) >= 20),
            summary,
        ),
        check(
            "仍保留必填人工确认字段或已完成确认",
            int(summary.get("仍缺必填字段数") or 0) > 0
            or (int(summary.get("仍缺必填字段数") or 0) == 0 and int(summary.get("原已填写且与候选一致数") or 0) >= 20),
            summary,
        ),
        check("差异预览不允许自动写入191", summary.get("是否允许自动写入191") is False, summary),
    ...
    log_dir = root / "04日志" / "单股证据核验191候选写入差异预览"
    stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output = log_dir / f"single-stock-evidence-191-candidate-write-diff-preview-verify-{stamp}.json"
    latest = log_dir / "single-stock-evidence-191-candidate-write-diff-preview-verify-最新.json"
