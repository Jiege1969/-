...
MD_PATH = DATA_DIR / "股票线第四批低风险续建队列_最新.md"
...
def main() -> None:
    ...
    for item in queue.get("construction_queue", []):
        if item.get("id") == "STOCK-AUTO4-002":
            item["status"] = "completed"
            item["validation_asset"] = "行业价格连续观测到复核字段联动样例验收结果_最新.json"
    ...
    for item in construction:
        lines.append(
            f"- {item['id']} | {item['title']} | {item['risk_level']} | {item['status']} | requires_user_confirmation={item['requires_user_confirmation']}"
        )
    lines.extend(["", "## 阻断登记", ""])
