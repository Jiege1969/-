# -*- coding: utf-8 -*-
"""
名称：验证单股证据核验正式回执待办卡.py
作用：验证214正式回执待办卡可生成，且不写正式205和191。
触发方式：手动验收、股票系统日常一键运行或C+++总验收调用。
依赖：生成单股证据核验正式回执待办卡.py、205/210/212状态文件。
所属系统：02杰哥扩展系统/01股票研究系统。
输出：04日志/单股证据核验正式回执待办卡/single-stock-evidence-formal-confirmation-receipt-todo-card-verify-最新.json。
安全边界：只运行214待办卡生成脚本并读取本地结果；不覆盖205，不覆盖191 CSV，不写191台账，不写172/175/178，不写正式档案，不触发n8n，不发送企业微信，不调用券商接口，不自动交易，不更新施工接续包。
创建/修改记录：2026-05-03 创建214正式回执待办卡验证脚本。
标识：single-stock-evidence-formal-confirmation-receipt-todo-card-verify
"""
...
def main() -> int:
    root = module_root()
    script = root / "02脚本" / "生成单股证据核验正式回执待办卡.py"
    ...
    tasks = report.get("待办事项", []) if isinstance(report.get("待办事项"), list) else []
    ...
    checks = [
        ...
        check("待办三条链路齐全", int(summary.get("待办链路数") or 0) == 3 and len(tasks) == 3, summary),
        check("每条链路给出四个填写字段", all(set(item.get("需要填写字段", [])) == {"确认结果", "核验状态", "核验人", "核验日期"} for item in tasks), tasks),
        check("本卡不写入205", summary.get("本卡是否写入205") is False, summary),
    ...
    log_dir = root / "04日志" / "单股证据核验正式回执待办卡"
    stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output = log_dir / f"single-stock-evidence-formal-confirmation-receipt-todo-card-verify-{stamp}.json"
    latest = log_dir / "single-stock-evidence-formal-confirmation-receipt-todo-card-verify-最新.json"
