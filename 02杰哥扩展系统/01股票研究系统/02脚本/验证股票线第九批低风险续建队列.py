...
RESULT_JSON = DATA_DIR / "股票线第九批低风险续建队列验收_最新.json"
RESULT_MD = DATA_DIR / "股票线第九批低风险续建队列验收_最新.md"


def main() -> None:
    errors: list[str] = []
    queue = json.loads(QUEUE_JSON.read_text(encoding="utf-8-sig")) if QUEUE_JSON.exists() else {}
    if not str(queue.get("asset_identity", "")).startswith("W1/W2"):
