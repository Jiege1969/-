...
ASSET_PATH = DATA_DIR / "股票线第六批统一影子验收总表_最新.json"
RESULT_JSON = DATA_DIR / "股票线第六批统一影子验收总表验收_最新.json"
RESULT_MD = DATA_DIR / "股票线第六批统一影子验收总表验收_最新.md"


def main() -> None:
    errors: list[str] = []
    asset = json.loads(ASSET_PATH.read_text(encoding="utf-8-sig")) if ASSET_PATH.exists() else {}
    if asset.get("asset_identity") != "W2第六批统一影子验收总表":
        errors.append("资产身份不正确")
    if asset.get("status") != "shadow_acceptance_summary":
        errors.append("状态不是shadow_acceptance_summary")
    for flag in ["not_formal_config", "not_entrypoint", "not_external_send", "not_score_write", "not_formal_rule_update"]:
    ...
    if summary.get("allow_shadow_acceptance_claim") is not True:
        errors.append("allow_shadow_acceptance_claim必须为true")
    for flag in ["allow_real_wecom_send", "allow_formal_entry", "allow_formal_score_update", "allow_auto_trade"]:
        if summary.get(flag) is not False:
            errors.append(f"{flag}必须为false")
    ...
    result = {
        ...
        "metrics": {
            ...
            "queue_completed_count": summary.get("queue_completed_count", 0),
