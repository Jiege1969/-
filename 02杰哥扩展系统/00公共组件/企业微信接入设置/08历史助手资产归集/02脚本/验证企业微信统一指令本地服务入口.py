...
作用：验证企业微信统一指令本地HTTP服务可用，并确认股票、系统状态、税收暂停和澄清入口均只在低风险本地范围内执行。
...
创建/修改记录：2026-04-29 创建企业微信统一指令本地服务入口验收脚本；2026-05-06 兼容系统管家确定性状态查询当前路由。
...
def main() -> int:
    ...
    stock = post_json(f"{SERVICE}/command", {"text": "新易盛"})
    ...
    add_check(checks, "股票指令可返回", stock.get("路由") == "股票研究" and "研究星级" in str(stock.get("回复", "")), stock)
    ...
    add_check(checks, "税收业务保持暂停", tax.get("路由") == "税收业务暂停" and tax.get("状态") == "已暂停", tax)
