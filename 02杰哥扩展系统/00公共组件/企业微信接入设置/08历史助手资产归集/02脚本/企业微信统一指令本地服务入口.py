...
def is_stock_question(message: str) -> bool:
    ...
    if not normalized:
        return False
    stock_words = (
        "股票", "股价", "个股", "a股", "研究信号", "走势", "k线", "行情", "财报", "估值",
        "新易盛", "贵州茅台", "宁德时代", "比亚迪", "天齐锂业", "中际旭创", "寒武纪",
    )
    if any(word in normalized for word in stock_words):
        return True
    if re.search(r"(?<!\d)(?:00|30|60|68)\d{4}(?!\d)", normalized):
        return True
    return bool(re.search(r"(分析|研究|看看|看一下|查一下).{0,12}(锂业|股份|科技|集团|银行|证券|能源|电力|药业|汽车)", normalized))
...
def build_transfer_payload(message: str, route_name: str, target_system: str, content: str) -> dict[str, Any]:
    return {
        "生成时间": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        "状态": "完成",
        "输入": message,
...
def build_stock_transfer_payload(message: str) -> dict[str, Any]:
    content = "\n".join([
        "【职责分流】这看起来是股票研究问题。",
        "请转给股票分析助手或股票分析专家处理；当前企业微信公共入口只做分流提示，不输出股票结论。",
        "安全边界：不请求19302股票业务接口、不接券商、不交易、不触发n8n、不真实发送企业微信。",
    ])
    return build_transfer_payload(message, "职责分流：股票分析助手", "02杰哥扩展系统/01股票研究系统", content)


def build_system_manager_transfer_payload(message: str) -> dict[str, Any]:
    content = "\n".join([
        "【职责分流】这属于系统状态或进度问题。",
        "请转给系统管家处理；视频助理只处理视频创作、预检、渲染和发布门禁相关问题。",
        "安全边界：不触发n8n、不真实发送企业微信、不写正式库。",
    ])
    return build_transfer_payload(message, "职责分流：系统管家", "00杰哥系统总管", content)
...
def execute_command(message: str) -> dict[str, Any]:
    ...
    if is_system_status_message(message):
        ...
        write_command_log(payload)
        return payload
    if is_stock_question(message):
        payload = build_stock_transfer_payload(message)
        write_command_log(payload)
        return payload
...
def execute_video_entry_command(message: str) -> dict[str, Any]:
    if is_system_status_message(message):
        payload = build_system_manager_transfer_payload(message)
        write_command_log(payload)
        return payload
    if is_stock_question(message):
        payload = build_stock_transfer_payload(message)
        write_command_log(payload)
        return payload
    return execute_video_assistant_command(message)
    ...
    def do_GET(self) -> None:
        ...
        if path == "/wecom/video-assistant":
            ...
            if not message:
                ...
                return
            self._send(json_response(execute_video_entry_command(message)))
            return
