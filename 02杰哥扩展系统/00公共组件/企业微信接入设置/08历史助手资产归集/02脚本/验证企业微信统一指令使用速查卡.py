...
作用：验证企业微信统一指令使用速查卡可生成，且内容覆盖股票、系统状态、知识库、内容办公、视频、税收暂停和澄清入口。
...
def main() -> int:
    ...
    required_routes = {"股票研究", "系统状态", "知识库问答", "内容办公处理", "视频制作", "税收业务暂停", "澄清一次"}
    ...
    add_check(checks, "股票推荐说法可直接使用", any(item.get("路由") == "股票研究" and item.get("推荐说法") == "新易盛" for item in card.get("指令", [])), card.get("指令", []))
    add_check(checks, "包含税收暂停边界", any(item.get("路由") == "税收业务暂停" for item in card.get("指令", [])), sorted(routes))
