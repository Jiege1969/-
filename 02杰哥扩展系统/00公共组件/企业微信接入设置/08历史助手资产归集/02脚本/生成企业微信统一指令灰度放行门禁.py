...
作用：读取企业微信助手状态摘要、统一指令路由预演和统一指令本地调用预演，生成是否允许进入小范围真实灰度的门禁报告。
触发方式：python 生成企业微信统一指令灰度放行门禁.py
依赖：Python标准库；企业微信统一指令灰度放行规则.json；企业微信助手状态摘要；统一指令路由预演；统一指令本地调用预演。
...
def main() -> int:
    ...
    add_check(checks, "企业微信助手系统状态摘要健康", status_summary.get("状态") == "healthy", status_summary)
    ...
    add_check(checks, "首轮灰度消息数不超过5条", int(gray_scope.get("最大首轮真实消息数", 99) or 99) <= 5, gray_scope)
    ...
    report = {
        ...
        "汇总": {
            "门禁状态": gate_status,
            "放行结论": "满足进入最多5条白名单真实灰度测试的前置条件" if gate_status == "pass" else "保持本地预演，不进入真实灰度",
