# -*- coding: utf-8 -*-
"""
名称：验证企业微信助手系统状态摘要.py
作用：验证企业微信助手系统状态摘要生成链路可运行，并确认统一指令路由预演、本地调用预演、灰度放行门禁、真实发送、Webhook、n8n触发等高风险动作仍关闭。
触发方式：python 验证企业微信助手系统状态摘要.py
依赖：Python标准库；生成企业微信助手系统状态摘要.py。
所属系统：02杰哥扩展系统/06企业微信助手系统
安全边界：只运行本地状态摘要生成和验收；只写入04日志；不写真实凭据；不触发Webhook；不触发n8n；不发送企业微信；不写旧系统；不接入税收业务；不接入交易。
创建/修改记录：2026-04-29 创建企业微信助手系统状态摘要验收脚本；2026-04-29 增加统一指令路由预演验收项；2026-04-29 增加统一指令本地调用预演验收项；2026-04-29 增加统一指令灰度放行门禁验收项；2026-04-29 增加统一指令使用速查卡验收项；2026-04-29 增加统一指令本地服务验收项。
...
def main() -> int:
    ...
    latest_md = root / "03数据" / "07状态摘要" / "企业微信助手系统状态摘要_最新.md"
    log_dir = root / "04日志"
    ...
    add_check(checks, "首轮真实消息上限不超过5", int(summary.get("最大首轮真实消息数", 99) or 99) <= 5, summary)
