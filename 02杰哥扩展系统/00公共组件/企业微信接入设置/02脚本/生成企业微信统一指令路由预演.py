...
def route_message(message: str, config: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "目标系统": fallback.get("目标系统", "00公共组件/企业微信接入设置"),
...
def answer_strategy(routed: dict[str, Any], sample: dict[str, Any]) -> str:
    explicit = str(sample.get("应答策略") or "").strip()
    if explicit:
        return explicit
    route_name = str(routed.get("命中路由") or "")
    source_robot = str(sample.get("入口机器人") or "").strip()
    target_robot = str(sample.get("期望机器人") or "").strip()
    if route_name == "澄清一次":
        return "简单澄清"
    if source_robot and target_robot and source_robot != target_robot:
        return "温和转向 + 递话术"
    if sample.get("信息不足") is True:
        return "追问缺失关键信息"
    return "直接回答"
...
def build_markdown(report: dict[str, Any]) -> str:
    ...
    for item in report.get("样例结果", []):
        lines.append(f"| {item['输入']} | {item['命中路由']} | {item.get('应答策略', '')} | {item['目标系统']} | {item['能力状态']} |")
    lines.extend([
        "",
        "## 安全边界",
        "",
...
def main() -> int:
    ...
    for sample in samples:
        ...
        routed["应答策略"] = answer_strategy(routed, sample)
