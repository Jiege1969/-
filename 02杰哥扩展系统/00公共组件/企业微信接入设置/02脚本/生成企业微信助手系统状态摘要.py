# -*- coding: utf-8 -*-
"""
名称：生成企业微信接入设置状态摘要.py
作用：汇总企业微信接入设置沙箱回环、凭据隔离、真实发送禁用态、统一指令路由预演、统一指令本地调用预演、灰度放行门禁和统一出口边界，生成日常状态摘要。
触发方式：python 生成企业微信接入设置状态摘要.py
...
安全边界：只读取企业微信接入设置本地日志和配置；只写入03数据/07状态摘要；不写真实凭据；不触发Webhook；不触发n8n；不发送企业微信；不写旧系统；不接入税收业务。
创建/修改记录：2026-04-29 创建企业微信接入设置状态摘要生成脚本；2026-04-29 纳入统一指令路由预演状态；2026-04-29 纳入统一指令本地调用预演状态；2026-04-29 纳入统一指令灰度放行门禁状态；2026-04-29 纳入统一指令使用速查卡状态；2026-04-29 纳入统一指令本地服务状态；2026-05-05 将本地系统健康与灰度放行状态解耦。
...
def build_markdown(report: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            f"- 股票L3接入对齐：{summary['股票L3接入对齐状态']}，验收 {summary['股票L3接入对齐通过数量']}/{summary['股票L3接入对齐总数']}",
...
def main() -> int:
    ...
    stock_l3_alignment_verify = run_local_script(root / "02脚本" / "验证企业微信股票L3接入对齐.py")
    ...
    stock_l3_alignment_report = load_json(root / "04日志" / "wecom-stock-l3-alignment-verify-最新.json", {})
    ...
    stock_l3_alignment_summary = stock_l3_alignment_report.get("汇总", {})
    stock_l3_alignment_ok = (
        stock_l3_alignment_verify.get("退出码") == 0
        and stock_l3_alignment_summary.get("状态") == "passed"
        and int(stock_l3_alignment_summary.get("失败", 1) or 0) == 0
    )
    local_system_ok = not failed_scripts and high_risk == 0 and disabled_ok and webhook_off and n8n_off and route_ok and local_call_ok and quick_card_ok and local_service_ok and stock_l3_alignment_ok
    ...
    report = {
        ...
        "汇总": {
            ...
            "股票L3接入对齐状态": stock_l3_alignment_summary.get("状态", "unknown"),
            "股票L3接入对齐通过数量": stock_l3_alignment_summary.get("通过", 0),
            "股票L3接入对齐总数": stock_l3_alignment_summary.get("总数", 0),
            "股票L3接入对齐失败数量": stock_l3_alignment_summary.get("失败", 0),
        ...
        "股票L3接入对齐验收": stock_l3_alignment_verify,
        "股票L3接入对齐报告": str(root / "04日志" / "wecom-stock-l3-alignment-verify-最新.json"),
        "安全边界": {
            "写入真实凭据": False,
            "企业微信真实发送": False,
            "触发Webhook": False,
    ...
    latest_md = output_dir / "企业微信接入设置状态摘要_最新.md"
    output_md = latest_md
