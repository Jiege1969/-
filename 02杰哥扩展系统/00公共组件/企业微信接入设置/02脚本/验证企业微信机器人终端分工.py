...
WECOM_ROOT = EXT_ROOT / "00公共组件" / "企业微信接入设置"
...
def main() -> int:
    ...
    required = ["杰哥股票短线分析助手", "杰哥股票分析专家", "杰哥系统管家", "杰哥工作秘书", "杰哥视频助理"]
    ...
    has_plain_secret = re.search(r"(?i)(secret|token|aeskey)[\\\"'\\s:：=]+[A-Za-z0-9_\\-]{20,}", config_text) is not None
    checks.append(check("总表不展开密钥明文", not has_plain_secret, str(CONFIG)))
    ...
    try:
        ...
        assistant_terms = ["![股票图形报告]", "分析对象", "结论", "观察条件", "风险线", "说明"]
        checks.append(check("股票短线助手显示符合个股研究短答要求", all(term in assistant_text for term in assistant_terms), assistant_text[:700]))
    except Exception as exc:
        assistant_text = ""
        checks.append(check("股票短线助手显示符合个股研究短答要求", False, str(exc)))
    ...
    try:
        ...
        expert_terms = ["【杰哥推荐】", "重点关注", "观察股票", "行业"]
    ...
    result = {
        ...
        "总数量": len(checks),
        "检查项": checks,
        "当前结论": "企业微信机器人终端分工机制已建立，股票短线助手/股票专家显示符合当前设置要求。" if not failed else "企业微信机器人终端分工仍有未通过项。",
        "安全边界": {
            "真实发送企业微信": False,
            "触发n8n": False,
            "调用OpenClaw": False,
