...
def main() -> int:
    ...
    json_path = output_dir / f"企业微信单股短回复_{name}_{timestamp}.json"
    latest_json = output_dir / "企业微信单股短回复_最新.json"
    md_path = output_dir / f"企业微信单股短回复_{name}_{timestamp}.md"
    latest_md = output_dir / "企业微信单股短回复_最新.md"
    log_path = log_dir / f"stock-wework-single-brief-reply-generate-{timestamp}.json"
