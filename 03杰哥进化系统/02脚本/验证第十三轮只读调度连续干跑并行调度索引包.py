...
ASSET_JSON = ROOT / "03数据" / "107第十三轮只读调度连续干跑并行调度索引包" / "第十三轮只读调度连续干跑并行调度索引包_最新.json"
LOG_DIR = ROOT / "04日志" / "第十三轮只读调度连续干跑并行调度索引包验收"
LATEST_LOG = LOG_DIR / "parallel-round13-readonly-scheduler-continuous-dryrun-verify-最新.json"
...
def main() -> int:
    ...
    if asset.get("状态") != "parallel_round13_readonly_scheduler_continuous_dryrun_ready":
        errors.append("状态不正确")
