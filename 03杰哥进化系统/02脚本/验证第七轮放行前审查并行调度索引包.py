...
ASSET_JSON = ROOT / "03数据" / "85第七轮放行前审查并行调度索引包" / "第七轮放行前审查并行调度索引包_最新.json"
LOG_DIR = ROOT / "04日志" / "第七轮放行前审查并行调度索引包验收"
LATEST_LOG = LOG_DIR / "parallel-round7-pre-release-review-verify-最新.json"
...
def main() -> int:
    ...
    if asset.get("状态") != "parallel_round7_pre_release_review_ready":
        errors.append("状态不正确")
