...
ASSET_JSON = ROOT / "03数据" / "88第八轮放行材料一致性并行调度索引包" / "第八轮放行材料一致性并行调度索引包_最新.json"
LOG_DIR = ROOT / "04日志" / "第八轮放行材料一致性并行调度索引包验收"
LATEST_LOG = LOG_DIR / "parallel-round8-release-material-consistency-verify-最新.json"
...
def _all_passed(items: Any) -> bool:
    return isinstance(items, list) and bool(items) and all(item.get("通过") is True for item in items if isinstance(item, dict))
...
def log_passed(path: Path) -> bool:
    ...
    if data.get("通过") is True and data.get("指标", {}).get("错误数", data.get("错误数", 0)) == 0:
        return True
    if data.get("验收结论") == "通过" and data.get("错误数", 0) == 0:
        return True
    ...
    if data.get("status") == "pass" and data.get("error_count", 0) == 0:
        return True
    if data.get("error_count") == 0 and _all_passed(data.get("检查结果", data.get("检查项", []))):
        return True
...
def main() -> int:
    ...
    if asset.get("状态") != "parallel_round8_release_material_consistency_ready":
        errors.append("状态不正确")
