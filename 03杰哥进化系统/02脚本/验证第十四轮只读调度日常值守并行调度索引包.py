...
ASSET_JSON = ROOT / "03数据" / "111第十四轮只读调度日常值守并行调度索引包" / "第十四轮只读调度日常值守并行调度索引包_最新.json"
LOG_DIR = ROOT / "04日志" / "第十四轮只读调度日常值守并行调度索引包验收"
LATEST_LOG = LOG_DIR / "parallel-round14-readonly-scheduler-daily-watch-verify-最新.json"
...
def main() -> int:
    ...
    if asset.get("状态") != "parallel_round14_readonly_scheduler_daily_watch_ready":
        errors.append("状态不正确")
