...
VERIFY_LOG = LOG_DIR / "full-delivery-low-risk-batch1-execute-verify-最新.json"

REQUIRED_FILES = [PACKAGE_JSON, PACKAGE_MD, EXEC_MD, RESULT_MD, NEXT_MD, GEN_LOG]
FORBIDDEN_MARKERS = [
    "允许真实发送",
    "允许真实触发",
    "允许接券商",
...
def main() -> int:
    ...
    add_check(
        checks,
        "总包状态就绪",
        package.get("状态") == "full_delivery_low_risk_batch1_executed",
        str(package.get("状态")),
    )
    ...
    exec_items = package.get("第一批低风险执行台账", [])
    exec_text = json.dumps(exec_items, ensure_ascii=False)
    exec_ok = len(exec_items) >= 6 and "未写正式规则" in exec_text and "未触发n8n" in exec_text and "未真实渲染" in exec_text
    add_check(checks, "第一批执行台账保持低风险", exec_ok, f"执行项={len(exec_items)}")

    results = package.get("第一批执行结果与证据", [])
    result_text = json.dumps(results, ensure_ascii=False)
    result_ok = len(results) >= 6 and "继续阻断真实渲染" in result_text and "基础链路保持通过" in result_text
    add_check(checks, "第一批证据结论有效", result_ok, f"证据项={len(results)}")

    next_items = package.get("第二批低风险续建建议", [])
    next_text = json.dumps(next_items, ensure_ascii=False)
    next_ok = len(next_items) >= 5 and "不生效" in next_text and "不导入" in next_text and "不渲染" in next_text
    add_check(checks, "第二批建议仍为低风险", next_ok, f"建议项={len(next_items)}")
