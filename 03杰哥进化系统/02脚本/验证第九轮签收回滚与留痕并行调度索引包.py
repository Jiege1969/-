...
ASSET_JSON = ROOT / "03数据" / "92第九轮签收回滚与留痕并行调度索引包" / "第九轮签收回滚与留痕并行调度索引包_最新.json"
LOG_DIR = ROOT / "04日志" / "第九轮签收回滚与留痕并行调度索引包验收"
LATEST_LOG = LOG_DIR / "parallel-round9-signoff-rollback-evidence-verify-最新.json"
...
def _all_passed(items: Any) -> bool:
    return isinstance(items, list) and bool(items) and all(item.get("通过") is True or item.get("passed") is True for item in items if isinstance(item, dict))
...
def main() -> int:
    ...
    if asset.get("状态") != "parallel_round9_signoff_rollback_evidence_ready":
        errors.append("状态不正确")
