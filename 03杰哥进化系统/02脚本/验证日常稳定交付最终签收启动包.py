...
VERIFY_LOG = LOG_DIR / "daily-stable-final-signoff-start-verify-最新.json"

REQUIRED_FILES = [PACKAGE_JSON, PACKAGE_MD, SIGNOFF_MD, START_MD, CONFIRM_MD, GEN_LOG]
REQUIRED_CONFIRM = ["19310", "19302", "企业微信", "n8n", "正式规则", "券商", "税局", "视频"]
FORBIDDEN_MARKERS = [
    "允许真实发送",
    "允许真实触发",
    "允许接券商",
...
def main() -> int:
    ...
    add_check(checks, "总包状态就绪", package.get("状态") == "daily_stable_final_signoff_start_ready", str(package.get("状态")))
    ...
    signoff = package.get("最终签收建议", [])
    signoff_text = json.dumps(signoff, ensure_ascii=False)
    signoff_ok = (
        len(signoff) >= 4
        and "日常可用交付版" in signoff_text
        and "稳定交付版" in signoff_text
        and "建议签收" in signoff_text
        and "建议进入试运行签收" in signoff_text
        and "真正自主运行版" in signoff_text
        and "暂不签收" in signoff_text
    )
    add_check(checks, "最终签收建议分层", signoff_ok, f"签收项={len(signoff)}")

    steps = package.get("试运行启动步骤", [])
    add_check(checks, "试运行启动步骤足量", len(steps) >= 7, f"启动步骤={len(steps)}")

    confirm = package.get("需总管确认后才能解锁事项", [])
    confirm_text = json.dumps(confirm, ensure_ascii=False)
    add_check(checks, "需总管确认事项覆盖红线", len(confirm) >= 7 and all(marker in confirm_text for marker in REQUIRED_CONFIRM), f"确认事项={len(confirm)}")
