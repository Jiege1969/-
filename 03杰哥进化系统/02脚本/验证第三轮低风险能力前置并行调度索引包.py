...
ASSET_JSON = ROOT / "03数据" / "73第三轮低风险能力前置并行调度索引包" / "第三轮低风险能力前置并行调度索引包_最新.json"
LOG_DIR = ROOT / "04日志" / "第三轮低风险能力前置并行调度索引包验收"
LATEST_LOG = LOG_DIR / "parallel-round3-low-risk-capability-prepare-verify-最新.json"
...
def main() -> int:
    ...
    if asset.get("状态") != "parallel_round3_low_risk_capability_prepare_ready":
        errors.append("状态不正确")
    if len(asset.get("并行任务", [])) < 3:
        errors.append("并行任务不足")
    for item in asset.get("并行任务", []):
        path = Path(item.get("验收日志", ""))
        if path.exists() and not verify_log_passed(path):
