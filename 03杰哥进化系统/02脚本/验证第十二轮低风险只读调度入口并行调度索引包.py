...
ASSET_JSON = ROOT / "03数据" / "103第十二轮低风险只读调度入口并行调度索引包" / "第十二轮低风险只读调度入口并行调度索引包_最新.json"
LOG_DIR = ROOT / "04日志" / "第十二轮低风险只读调度入口并行调度索引包验收"
LATEST_LOG = LOG_DIR / "parallel-round12-low-risk-readonly-scheduler-entry-verify-最新.json"
...
def main() -> int:
    ...
    if asset.get("状态") != "parallel_round12_low_risk_readonly_scheduler_entry_ready":
        errors.append("状态不正确")
