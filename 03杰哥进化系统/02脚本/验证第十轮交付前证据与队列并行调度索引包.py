...
ASSET_JSON = ROOT / "03数据" / "95第十轮交付前证据与队列并行调度索引包" / "第十轮交付前证据与队列并行调度索引包_最新.json"
LOG_DIR = ROOT / "04日志" / "第十轮交付前证据与队列并行调度索引包验收"
LATEST_LOG = LOG_DIR / "parallel-round10-pre-delivery-evidence-queue-verify-最新.json"
...
def main() -> int:
    ...
    if asset.get("状态") != "parallel_round10_pre_delivery_evidence_queue_ready":
        errors.append("状态不正确")
