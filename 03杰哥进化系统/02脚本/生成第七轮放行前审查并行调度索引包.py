...
OUTPUT_DIR = EVOLUTION_ROOT / "03数据" / "85第七轮放行前审查并行调度索引包"
LATEST_JSON = OUTPUT_DIR / "第七轮放行前审查并行调度索引包_最新.json"
LATEST_MD = OUTPUT_DIR / "第七轮放行前审查并行调度索引包_最新.md"
...
TASKS: list[dict[str, Any]] = [
    {
        ...
        "业务域": "进化系统",
        "目标": "把候选状态机推进到正式规则申请草案审查，但不写正式规则。",
        "验收日志": str(
            EVOLUTION_ROOT
            / "04日志"
            / "三业务反馈候选正式规则申请草案审查包验收"
            / "three-business-formal-rule-application-draft-review-verify-最新.json"
        ),
    },
    {
        "编号": "ROUND7-T",
        "名称": "n8n离线导入前凭据隔离与启用禁入包",
        "业务域": "进化系统",
        "目标": "把 n8n 离线失败演练推进到导入前凭据隔离和启用禁入。",
        "验收日志": str(
            EVOLUTION_ROOT
            / "04日志"
            / "n8n离线导入前凭据隔离与启用禁入包验收"
            / "n8n-offline-import-credential-isolation-no-activate-verify-最新.json"
        ),
    },
    {
        "编号": "ROUND7-U",
        ...
        "业务域": "视频制作系统",
        "目标": "把视频试运行禁入推进到白名单草案和回滚预案草案，仍不生效。",
        "验收日志": str(
            ROOT
            / "02杰哥扩展系统"
            / "02视频制作系统"
            / "04日志"
            / "真实渲染试运行白名单与回滚预案草案包验收"
