...
ASSET_JSON = ROOT / "03数据" / "119第十五轮跨日值守样本并行调度索引包" / "第十五轮跨日值守样本并行调度索引包_最新.json"
LOG_DIR = ROOT / "04日志" / "第十五轮跨日值守样本并行调度索引包验收"
LATEST_LOG = LOG_DIR / "parallel-round15-cross-day-watch-sample-verify-最新.json"
...
def main() -> int:
    ...
    if asset.get("状态") != "parallel_round15_cross_day_watch_sample_ready":
        errors.append("状态不正确")
    if len(asset.get("并行任务", [])) < 3:
        errors.append("并行任务不足")
    for item in asset.get("并行任务", []):
        data_dir = Path(item.get("数据目录", ""))
