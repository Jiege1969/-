...
ASSET_JSON = ROOT / "03数据" / "123第十六轮周周期值守趋势并行调度索引包" / "第十六轮周周期值守趋势并行调度索引包_最新.json"
LOG_DIR = ROOT / "04日志" / "第十六轮周周期值守趋势并行调度索引包验收"
LATEST_LOG = LOG_DIR / "parallel-round16-week-cycle-watch-trend-verify-最新.json"
...
def main() -> int:
    ...
    if asset.get("状态") != "parallel_round16_week_cycle_watch_trend_ready":
        errors.append("状态不正确")
