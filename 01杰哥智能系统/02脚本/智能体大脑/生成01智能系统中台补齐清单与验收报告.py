...
READ_SOURCES = {
    "本轮并行施工派工包Markdown": MANAGER_ROOT / "03数据" / "开工上下文" / "本轮并行施工派工包_最新.md",
    "本轮并行施工派工包JSON": MANAGER_ROOT / "03数据" / "开工上下文" / "本轮并行施工派工包_最新.json",
...
GATE_FILES = {
    "股票分析系统硬闸门配置": SYSTEM_ROOT / "02杰哥扩展系统" / "01股票研究系统" / "01配置" / "股票分析系统硬闸门配置.json",
    "股票自动交易屏蔽总闸门规则": MANAGER_ROOT / "01配置" / "股票自动交易屏蔽总闸门规则.json",
    "股票分析非交易边界规则": MANAGER_ROOT / "01配置" / "股票分析非交易边界规则.json",
}
...
def hard_gate_summary() -> dict[str, Any]:
    package = load_json(READ_SOURCES["本轮并行施工派工包JSON"], {})
    dispatch_gate = package.get("最高优先硬闸门", {})
    gate_reports = []
    all_closed = True
    for name, path in GATE_FILES.items():
        data = load_json(path, {})
        switches = data.get("硬性关闭开关", {})
        enabled = {key: value for key, value in switches.items() if value is True}
        ...
        gate_reports.append(
            {
                ...
                "路径": str(path),
                "存在": path.exists(),
                "硬性关闭开关数量": len(switches),
                "误开启项": enabled,
                "当前状态": data.get("当前状态", ""),
            }
        )
    return {
        "派工包系统标准": dispatch_gate.get("系统标准", ""),
        "硬性关闭": dispatch_gate.get("硬性关闭", []),
        "必须拦截": dispatch_gate.get("必须拦截", []),
        "允许研究表达": dispatch_gate.get("允许研究表达", []),
        "闸门文件检查": gate_reports,
        "结论": "通过" if all_closed else "失败",
        "说明": "股票系统只按分析系统标准运行；本轮不恢复、不设计自动交易、券商接口、下单、资金账户、交易工作流、买卖/仓位/目标价建议。",
    }
...
def build_samples() -> list[dict[str, Any]]:
    ...
    for question in questions:
        ...
        stock_trade_intent = any(marker in question for marker in ["券商", "下单", "买入", "卖出", "能不能买", "仓位", "目标价"])
        high_risk = any(
            marker in question
            for marker in [
                "删除",
        ...
        if stock_trade_intent:
            task["任务类型"] = "股票交易意图阻断"
            task["风险等级"] = "高"
            task["命中关键词"] = sorted(set(task.get("命中关键词", []) + ["股票硬闸门", "交易意图"]))
            capability["能力名"] = "股票硬闸门拦截"
            capability["归属系统"] = "01杰哥智能系统"
            capability["入口类型"] = "本地只读验收"
            capability["默认执行方式"] = "阻断"
            capability["任务风险等级"] = "高"
            capability["需要人工确认"] = True
            capability["当前动作"] = "按股票分析系统硬闸门阻断，不执行外部工具"
        samples.append(
            {
                "问题": question,
                "任务识别": task,
...
def build_markdown(report: dict[str, Any]) -> str:
    lines = [
        ...
        "## 最高优先硬闸门继承",
        "",
        f"- 结论：{report['最高优先硬闸门继承']['结论']}",
        f"- 系统标准：{report['最高优先硬闸门继承']['派工包系统标准']}",
        f"- 说明：{report['最高优先硬闸门继承']['说明']}",
        "- 闸门文件：",
    ]
    for item in report["最高优先硬闸门继承"]["闸门文件检查"]:
        lines.append(f"  - {item['名称']}：存在={item['存在']}；误开启项={len(item['误开启项'])}")
    lines.extend([
        "",
        "## 能力清单",
        "",
