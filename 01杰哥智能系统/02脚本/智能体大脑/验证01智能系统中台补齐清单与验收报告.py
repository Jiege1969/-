...
def main() -> int:
    ...
    hard_gate = report.get("最高优先硬闸门继承", {})
    ...
    add_check(checks, "继承股票分析系统最高优先硬闸门", hard_gate.get("结论") == "通过" and all(item.get("存在") and not item.get("误开启项") for item in hard_gate.get("闸门文件检查", [])), hard_gate)
    add_check(checks, "股票交易意图样本已阻断", any(("券商" in item.get("问题", "") or "下单" in item.get("问题", "") or "能不能买" in item.get("问题", "")) and item.get("异常处理", {}).get("是否高风险") is True and "阻断" in item.get("异常处理", {}).get("处理方式", "") for item in samples), samples)
    ...
    add_check(checks, "未输出买卖仓位目标价建议", safety.get("输出买卖/仓位/目标价建议") is False and summary.get("输出买卖仓位目标价建议") is False, safety)
